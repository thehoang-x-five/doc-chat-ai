"""
Retry and resilience utilities for RAG Pipeline.
...
Adapted from upstream RAGAnything v1.2.10 resilience module.

Usage::

    from app.services.rag_patterns.pipeline.resilience import retry, async_retry, CircuitBreaker

    @async_retry(max_attempts=5, base_delay=2.0)
    async def call_llm(prompt: str) -> str:
        return await aclient.chat.completions.create(...)

    llm_breaker = CircuitBreaker(failure_threshold=5, reset_timeout=60.0, name="llm")

    @llm_breaker.async_call
...
class CircuitBreaker:
    ...
    Args:
        ...
        failure_exceptions: Exception types treated as upstream failures.
    """
